...
(function($){
  ...
  var _validationTypes = {
    required: {msg: ' es obligatorio completarlo' },
    tel: {msg: ' no es un número de teléfono válido' },
    email: {msg: ' no es una dirección de correo válida' },
    date: {msg: ' no es una fecha válida'},
    number: {msg: ' no es un número válido'}
  };
  ...
  function setupFormValidation(options){
    settings = $.extend({
      ...
      errorSummaryMsg: 'Por favor verifica los siguientes datos erróneos antes de continuar:',
      validateTel: true,
      telRegex: /^\+*[\d-()]{7,20}$/,
      validateEmail: true,
      emailRegex: /^(\S+@\S+)*$/,
